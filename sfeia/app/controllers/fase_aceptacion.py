...
class FaseAceptacion:
    ...
    def ejecutar(self) -> dict:
        ...
        for i, w in enumerate(ventanas):
            ventana = fa.ejecutar_scan_ventana(w, scan_cfg, historial=hist)
            if ventana.get("error"):
                errores += 1
                continue
            for c in ventana["combinaciones"]:
                fila = {
        ...
        # dedupe preservando orden (primero las candidatas)
        vistos: set[str] = set()
        a_confirmar_uniq: list[dict] = []
        for f in a_confirmar:
            k = (f["estudio_ini"], f["estudio_fin"], f["segmento"], f["estrategia"])
            if k in vistos:
                continue
            vistos.add(k)
            a_confirmar_uniq.append(f)

        confirmadas: list[dict] = []
        for f in a_confirmar_uniq:
            conf = self._confirmar(f, scan_cfg)
            if conf:
    ...
    # ------------------------------------------------------- confirmación
    def _confirmar(self, f: dict, scan_cfg: dict) -> dict | None:
