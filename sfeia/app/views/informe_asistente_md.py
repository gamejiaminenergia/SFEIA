...
def _tabla_robustez(r: dict) -> str:
    ...
    if dups:
        texto += "\n\n> ⚠️ **Maestros con comportamiento idéntico** (misma mediana y media de margen): "
        texto += "; ".join(" y ".join(d) for d in dups)
        texto += ". Cuántos como 'independientes' infla el top-N."
    return texto
...
def _seccion_decision(r: dict, res) -> str:
    ...
    L: list[str] = []
    L.append("## 10. Decisiones de negocio para el agente XXXC")
    L.append("")
    ...
    bullets.append(
        ...
        " de un solo período. Revisa esta decisión cada ventana."
    )
    for b in bullets:
        L.append(f"- {b}")
    L.append("")
...
def renderizar(resultado: dict, cfg: dict) -> str:
    ...
    L.append("## 4. Escenarios de estrés (sintéticos — la ventana no tuvo estos días)")
    L.append("")
    ...
    L.append("## 5. Frecuencia histórica de escasez (todo el histórico de la BD)")
    L.append("")
    ...
    L.append("## 6. Distribución de márgenes en el impacto (riesgo)")
    L.append("")
    ...
    # ---- 7. Simulación diaria ----
    L.append(f"## 7. Simulación día a día ({v['impacto']['ini']} → {v['impacto']['fin']})")
    L.append("")
    L.append("XXXC replica cada día el perfil recomendado y se calcula su margen y garantía estimada con la misma"
             ...
             " maestros y del segmento ese día.")
    L.append("")
    L.append(_tabla_simulacion(resultado))
    L.append("")

    # ---- 8. Balance del impacto ----
    L.append("## 8. Balance del impacto")
    L.append("")
    L.append(_tabla(
    ...
    # ---- 9. Advertencias ----
    L.append("## 9. Advertencias y limitaciones")
    L.append("")
