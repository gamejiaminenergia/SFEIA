...
- Dependencias (psycopg2-binary, pyyaml, pytest, requests, scikit-learn, scipy) solo existen en `.venv`; el Python del sistema no las tiene.
...
- **Ventanas por defecto:** estudio = últimos `dias_estudio_por_defecto: 30` días antes del impacto; impacto = últimos `dias_impacto_por_defecto: 7` días de la ventana principal (config.yaml). El CLI (`--estudio-ini`, `--impacto-fin`, …) sobreescribe el config.
