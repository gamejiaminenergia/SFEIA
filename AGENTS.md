...
- Harness de aceptación (Fase 0 del plan de aceptación): `./.venv/bin/python -m sfeia.main --aceptacion` corre el grid de ventanas 2015→hoy (estudio 90 d + impacto 7 d, paso 30 d), hace un scan grueso de todas las (segmento × estrategia × ventana) con scorecard N0–N4 y confirma candidatas con el flujo completo. Exporta `docs/scorecard_aceptacion.md` + `data/scorecard{,_confirmadas}.csv`. Ver `docs/plan_aceptacion_imitador.md`.
...
- **Veredicto de aceptación (Fase 0–1 del plan):** el harness `--aceptacion` barrió 138 ventanas × 984 combinaciones (2015→2026) y **ninguna** pasó el gate V1 (bootstrap p≤0.05; mínimo observado 0.46). Veredicto: **NO-OPERABLE** con el premio C16–C18 relativo (los agentes de una misma estrategia son conductualmente homogéneos → el top es suerte entre iguales). No se deben relajar umbrales para forzar un pase; la evidencia vive en `docs/scorecard_aceptacion.md`.
