...
- Harness de aceptación (Fase 0 del plan de aceptación): `./.venv/bin/python -m sfeia.main --aceptacion` corre el grid de ventanas 2015→hoy (estudio 90 d + impacto 7 d, paso 30 d), hace un scan grueso de todas las (segmento × estrategia × ventana) con scorecard N0–N4 y confirma candidatas con el flujo completo. Exporta `docs/scorecard_aceptacion.md` + `data/scorecard{,_confirmadas}.csv` (+ `data/scorecard_arquetipos{,_confirmadas}.csv` en modo arquetipo). Ver `docs/plan_aceptacion_imitador.md`. El experimento de poder (Ruta C) se corre con `--poder` → `docs/experimento_poder_estadistico.md`.
...
- **Salidas en la raíz:** `docs/informe_asistente_imitador.md` + `data/{maestros_topN, politica_clonacion, simulacion_impacto, resumen_impacto, distribucion_margenes, escenario_escasez, historial_escasez, validacion_seleccion}.csv` (y `data/walk_forward.csv` con `--walk-forward`, `data/barrido_combinaciones.csv` + `docs/informe_barrido_combinaciones.md` con `--barrido`, `docs/scorecard_aceptacion.md` + `data/scorecard{,_confirmadas}.csv` con `--aceptacion`). El informe incluye medidas de riesgo (distribución, drawdown, garantías), escenarios de estrés de escasez, frecuencia histórica de escasez (2015→hoy), robustez, **validez de la selección** (bootstrap/holdout/DSR/replicación) y **alternativa recomendada** si la combinación pedida no valida.
- **Veredicto de aceptación (Fase 0–1 del plan):** el harness `--aceptacion` barrió 138 ventanas × 984 combinaciones (2015→2026) en modo topN y modo **arquetipo** (Ruta A: clona el arquetipo ganador por régimen). Tras corregir un **bug en `bootstrap_skill_luck`** (el estadístico real era el mínimo en vez del máximo, lo que inflaba p≈1.0), el veredicto es **OPERABLE**: 20 combinaciones confirmadas en modo topN y 25 en modo arquetipo (p≤0.001–0.03, réplica ≥0.5, DSR ≥0.95, holdout ≥50 %, E1≥60 %, EV>0), dominadas por **Comercializador regulado** (PEQUEÑO/MEDIANO) y **Trader no regulado** (MEDIANO) en 2015–2021. 2023–2025 (escasez) siguen fallando: la operabilidad es por régimen. El experimento `--poder` (Ruta C) confirma que la señal no es un artefacto de poder (n_boot=2000, ventana 365 d y pool entre segmentos mantienen p≤0.017). El walk-forward de 84 ventanas (2015→2022) muestra skill persistente (p≤0.05 en 99 % de ventanas) pero la operabilidad completa es **por ventana** (44 %): se debe re-seleccionar con `--walk-forward`. Evidencia en `docs/scorecard_aceptacion.md` + `docs/experimento_poder_estadistico.md`.
...
- **Determinismo SQL**: `d1_diario.sql` toma los precios de sistema con `MAX(ds.*)` (no `AVG`) y `f-1_segmentacion.sql` redondea las sumas a 3 decimales. `fact_daily_sistema` tiene una fila por fecha → MAX == valor; esto evita la variación de 1 ULP que la agregación paralela de PostgreSQL introducía entre corridas (el scorecard N0.2 exige byte-a-byte idéntico).
